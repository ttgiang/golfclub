...
@Component({
  ...
  providers: [ClubsService, AdaptService, CommonService, DatePipe],
})
  ...
  ngOnInit() {
    ...
    this.navigate = this.route.params.subscribe((params: any) => {
      ...
      if (environment.thanh) {
        const data = that.clubsService.getFoundClubs(
          that.searchingParams.location
        );
        that.clubsService.setClubsData([data]);
      } else {
        that.clubsService
          .getFoundClubs(that.searchingParams.location)
