...
export class SearchPanelComponent {
  ...
  constructor(
    ...
    if (environment.thanh) {
      console.log("[search.thanh.true]", environment.thanh);
      that.citiesList = cities;
    } else {
      console.log("[search.thanh.false]", environment.thanh);
      cities.done(function (data: any) {
        that.citiesList = data;
      });
    }

    this.maxStartDate = this.commonService.addDays(
      new Date(),
      MAX_NUMBER_OF_DAYS
    );
  ...
  fieldChanged(e: any) {
    if (e.dataField === "startDate") {
      ...
      this.maxEndDate = this.commonService.addDays(
        value,
        MAX_NUMBER_OF_DAYS_FOR_BOOKING
      );
      if (
        !this.data.endDate ||
        this.data.endDate <= this.data.startDate ||
        this.data.endDate >
          this.commonService.addDays(value, MAX_NUMBER_OF_DAYS_FOR_BOOKING)
      ) {
