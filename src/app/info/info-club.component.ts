...
@Component({
  ...
  providers: [CommonService, AdaptService, DatePipe],
})
export class InfoComponent implements OnInit {
  ...
  ngOnInit() {
    if (environment.thanh) {
      this.club = this.clubsService.getClubById(this.searchingParams.clubId);
      this.clubsService.setClubsData([this.club]);
      this.loadingData = false;
    } else {
      this.clubsService
        .getClubById(this.searchingParams.clubId)
        .done((data: any) => {
          this.club = data;
